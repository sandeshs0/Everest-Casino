...
var Deck = {
  // Deck Object - A 52 card poker deck
  ...
  Initialize: function () {
    ...
    for (
      var id = 0, i = 0;
      i < 4;
      i++ // for each Suit {Clubs,Diamonds,Hearts,Spades}
    )
      for (
        var j = 0;
        j < 13;
        j++ // for each Rank {A,2,...,10,J,Q,K}
      )
        this.Cards.push(
          new Card(
            (++id).toString(),
  ...
  Deal: function (numCards) {
    ...
    for (var i = 0; i < numCards; i++) dealt.push(this.Shuffled.pop());
    return dealt;
  },
};

function Hand(cards) {
  // Hand object - The player's active Card objects
  this.Cards = cards; // Array of Card objects
  this.Evaluate = function () {
    // Return ID of winning hand type, or -1 if losing hand
  ...
  this.IsRoyal = function () {
    // true if card ranks: 10, 11, 12, 13, 1 (Ten, Jack, Queen, King, Ace)
  ...
  this.IsFullHouse = function () {
    // true if three card ranks equal with remaining two card ranks equal
    var sorted = this.Sort();
    return (
      (sorted[0].Rank === sorted[1].Rank &&
        sorted[2].Rank === sorted[4].Rank) ||
      (sorted[0].Rank === sorted[2].Rank && sorted[3].Rank === sorted[4].Rank)
    );
  };
  this.IsFourOfAKind = function () {
    // true if four card ranks equal
    var sorted = this.Sort();
    return (
      sorted[0].Rank === sorted[3].Rank || sorted[1].Rank === sorted[4].Rank
    );
  };
  this.IsFlush = function () // true if all suits equal
  {
    for (var i = 1; i < this.Cards.length; i++)
      if (this.Cards[i].Suit !== this.Cards[0].Suit) return false;
    return true;
  };
  this.IsStraight = function () {
    // true if successively incremented ranks (n, n + 1,... n + 4) or royal (10, 11, 12, 13, 1)
    if (this.IsRoyal())
      // royal straight
      return true;
    var sorted = this.Sort();
  ...
  this.IsThreeOfAKind = function () {
    ...
    for (var i = 0; i < this.Cards.length; i++) {
      if (CardCounts.hasOwnProperty(this.Cards[i].Rank))
        // Check if key exists in hashmap
        CardCounts[this.Cards[i].Rank]++; // Increment frequency
      else CardCounts[this.Cards[i].Rank] = 1; // First occurence
    }
    for (
      var i = 1;
      i < 14;
      i++ // Check each rank, 1-13 (Ace-King)
    )
      if (CardCounts.hasOwnProperty(i) && CardCounts[i] >= 3)
        // at least triple of a rank
        return true;
    return false;
  };
  this.IsTwoPair = function () {
    ...
    var hasPair = false;
    for (var i = 1; i < 14; i++) {
      // Check each rank, 1-13 (Ace-King)
      if (CardCounts.hasOwnProperty(i) && CardCounts[i] === 4)
        // four of a kind is arguably a special case of two pair
        return true;
      else if (CardCounts.hasOwnProperty(i) && CardCounts[i] >= 2) {
        // at least double of a rank
  ...
  this.IsJacksOrBetter = function () {
    ...
    for (
      ...
      if (CardCounts.hasOwnProperty(i) && CardCounts[i] >= 2) return true;
    return false;
  };
  this.Sort = function () {
    // Returns new array in sorted rank, precedence: 1-13 (Ace-King)
    var sorted = new Array();
    for (var i = 0; i < this.Cards.length; i++) sorted.push(this.Cards[i]);
...
function _DealClick() {
  ...
  if (_GameState === GameStates.GameOver) {
    ...
    document.getElementById("btnDeal").innerHTML = "DEAL";
    document.getElementById("btnBetDown").className = document.getElementById(
      "btnBetUp"
    ).className = "BUTTON ROUNDED";
    for (
      var i = 0;
      i < _Hand.Cards.length;
      i++ // Flip last losing hand face down to signify new game
    )
...
function _OnCanvasClick(e) {
  // Canvas (player's Hand) click event handler
  if (_GameState !== GameStates.SecondDeal)
    // Card interaction available on second deal only
    return;
  e = e || window.event;
  e.preventDefault();
  for (var i = 0; i < 5; i++) {
    // Check collision/intersection of mouse click on each Card
    var cardX = _HandX + (i * (_CardWidth + 4) + 4); // 4px buffer
    if (
      e.offsetX >= cardX && // Inside left border
      e.offsetX <= cardX + _CardWidth && // Inside right border
      e.offsetY >= 0 && // Below card top
      e.offsetY <= _CardHeight
    ) {
      // Above card bottom
      _Hand.Cards[i].Locked = !_Hand.Cards[i].Locked; // Collision detected, toggle card lock
      if (_Hand.Cards[i].Locked) GameAudio.Play("Hold");
      else GameAudio.Play("Unhold");
    }
  }
  _DrawScreen();
}
...
function _ProcessHand() {
  // Process a Hand checking for win, loss, or game over
  var winID = _Hand.Evaluate();
  if (winID != -1) {
    // Woo-Hoo, a winning hand
    _GameState = GameStates.HandWon;
    ...
    // Losing Hand
    if (_Credits === 0) {
      // Check for termination condition (0 Credits)
    ...
    if (_CurrentBet > _Credits)
      // Ensure sufficient credits for bet
      _CurrentBet = _Credits; // Reduce bet to match remaining credits
  }
}

function _GetPrizeAmount(winID) {
  // Payout schedule
...
function _DrawScreen() {
  ...
  if (_GameState == GameStates.Uninitialized)
    // Redrawn only if loading screen is down
    return;
  var g = _Canvas.getContext("2d"); // Graphics context
  g.clearRect(0, 0, _Canvas.width, _Canvas.height); // Wipe frame clean
  for (var i = 0; i < _Hand.Cards.length; i++) {
    // for each Card in Hand
    if (_Hand.Cards[i].FlipState === 1) _DrawCardFace(g, i); // FlipState == 1
    else _DrawCardBack(g, i); // FlipState == 0
...
function _UpdateCreditsLabel() {
  document.getElementById("divCredits").innerHTML =
    this._Credits.toString() + " CREDITS";
}
...
function _UpdateBetLabel() {
  if (
    ...
    // Betting buttons lit up and active
    document.getElementById("btnBetDown").className = document.getElementById(
      "btnBetUp"
    ).className = "BUTTON ROUNDED";
  else if (
    _GameState === GameStates.SecondDeal ||
    _GameState === GameStates.GameOver
  )
    // Betting buttons subdued and inactive
    document.getElementById("btnBetDown").className = document.getElementById(
      "btnBetUp"
    ).className = "BUTTON_OFF ROUNDED";

  document.getElementById("lblCurrBet").innerHTML =
    "BET " + _CurrentBet.toString();
}

function _PrizeWinBlink() {
  // Handles marquee blink on winning prize row
...
function _ResetPrizeWin() {
  ...
  if (_WinID !== -1) {
    // Party is over, back to normal row style
...
function _MeasureUI() {
  // Set position and size rendering information for UI elements
  ...
  _CardWidth = Math.floor(windowWidth / 10.5); // Drawn card size is proportional to window size
  ...
  document.getElementById("tblMarquee").style.fontSize =
    marqueeFontSize.toString() + "px"; // apply font
  document.getElementById("tblMarquee").style.width =
    (_CardWidth + 4) * 5 + "px";
...
function _DrawCardBack(g, cardIndex) {
  ...
  // g.fillStyle = " #cccccc"; // Set dark red card back
  // g.borderRadius = "5px";
  // g.style.borderRadius = "2px";

  var cardX = _HandX + (cardIndex * (_CardWidth + 4) + 4); // Card x position (4px buffer)
  g.strokeStyle = "gray";
  g.strokeRect(cardX, 0, _CardWidth, _CardHeight); // Render card back
...
function _DrawHandOverMessage(g) {
  ...
  g.globalAlpha = 0.8; // Set semi-transparent background color
  ...
  g.fillStyle = _GameState == GameStates.HandWon ? "#33cc33" : "#cc0000  "; // Win = gold, Lose = red
  g.strokeStyle = "#FFFFFF"; // Set black border color
...
function _DrawRoundedRectangle(
  g,
  x,
  y,
  width,
  ...
  // Utility for drawing a rounded rectangle to a given graphics context. Args->(graphics, x, y, width, height, borderRadius, doFill, doStroke)
...
var GameAudio = {
  ...
  Initialize: function () {
    ...
    var extension = oggCapable
      ? "ogg"
      : mp3Capable
      ? "mp3"
      : wavCapable
      ? "wav"
      : ""; // Set audio type by precendence
    ...
    for (var i = 0; i < this._FileNames.length; i++) {
      ...
      for (
        ...
        bufferIndex++ // Buffered effects are required for "async", overlapping play of same sound
      ) {
  ...
  Play: function (soundName) {
    if (this._SoundEffects.hasOwnProperty(soundName)) {
      // Check if sound effect is keyed in hashmap
...
function _OnResourceLoaded() {
  // Fired per loaded resource
  _NumResourcesToLoad--; // Decrement global counter
  if (_NumResourcesToLoad === 0)
    // All resources loaded, begin game
    _NewGame();
}
...
window.onbeforeunload = function () {
  if (_PrizeWinThread != null)
    // If marquee blinking effect is running when app is closing
